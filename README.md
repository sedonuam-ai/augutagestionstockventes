...
3. Sur la page du dépôt, clique sur **Add file → Upload files**, puis glisse-dépose **tous les fichiers** de ce zip (10 fichiers au total : `index.html`, `style.css`, `app.js`, `manifest.json`, `sw.js`, `README.md` et 5 images `.png`). Tous les fichiers sont à plat, sans sous-dossier — c'est fait exprès pour que l'upload depuis un téléphone ne pose jamais de souci.
...
├── app.js                  → logique métier (stock, PMP, ventes, marges, TVA, devis/factures)
...
├── icon-512-maskable.png   → logo adaptatif (Android)
├── apple-touch-icon.png    → logo (iOS/Safari)
├── favicon-32.png          → icône d'onglet navigateur
└── README.md
```

Tous les fichiers sont volontairement à la racine, sans sous-dossier : c'est le format le plus fiable pour un envoi depuis un téléphone (l'upload de dossiers depuis mobile échoue souvent sur GitHub).
...
- **Couleurs / logo** : `style.css` (variables CSS en haut du fichier) et les fichiers `icon-*.png` / `apple-touch-icon.png` / `favicon-32.png` à la racine.
